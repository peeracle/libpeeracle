...
# WebRTC when built as part of Chromium and when it is built without Chromium.
{
  ...
  'targets': [
    ...
    {
      'target_name': 'peeracle_storage_unittest_apk',
      'type': 'none',
      'variables': {
        'test_suite_name': 'peeracle_storage_unittest',
        'input_shlib_path': '<(SHARED_LIB_DIR)/<(SHARED_LIB_PREFIX)peeracle_storage_unittest<(SHARED_LIB_SUFFIX)',
      },
      'dependencies': [
        '<(DEPTH)/peeracle/Storage/Storage.gyp:peeracle_storage_unittest',
      ],
      'includes': [
        '../../third_party/webrtc/build/apk_test.gypi',
      ],
